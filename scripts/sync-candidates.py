...
SIDOS = [
    # 2026 광주·전남 행정통합 (NEC 검증 2026-06-16):
    #   - 광역직(시·도지사 tc=3, 교육감 tc=11): NEC가 '전남광주통합특별시'로 통합 선출
    #     → sdName='광주광역시'/'전라남도' 어느 쪽으로 조회해도 동일 명단을 통합명으로 반환.
    #   - 기초직(구청장·시장·군수 tc=4, 지방의원 tc=5·6): NEC가 '광주광역시'/'전라남도'로
    #     정상 분리 반환 (별개 자치단체 유지). 엑셀(제9회 선거구)도 별개 시트.
    #   - 인천: 중구·동구 폐지, 검단·영종·제물포구 신설 (wiwName 단위).
    # 세 sdName을 모두 호출하고 huboid 기준 dedup → 광역직 통합/기초직 분리가 자연스럽게 유지됨.
    # ⚠️ SIDO_NORMALIZE(옛 정규화)는 기초직까지 통합명으로 덮어 v3에서 광주/전남 0명 버그를
    #    유발해 제거함(2026-06-16). voice.html은 별도 매칭이라 영향 없음.
...
def main():
    ...
    # (sido 정규화 제거됨 2026-06-16) NEC가 반환하는 sdName 그대로 사용:
    # 광역직='전남광주통합특별시'(통합), 기초직='광주광역시'/'전라남도'(분리).
