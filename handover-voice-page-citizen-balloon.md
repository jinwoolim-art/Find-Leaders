...
1. **`시민의풍선` 시트 row 절대 삭제 금지** — 이미 외부(카톡·SNS·영상)에 노출된 카운트
2. **카운터 reset 금지** — 0부터 다시 시작하면 시민 신뢰 손상
3. **신규 컬럼 추가는 시트 우측만** — 기존 row 영향 0
4. **listVoices 카운트는 누적 유지** (`lastRow - 1`)
5. **페이지 내 표시 URL** (`www.illkkun.cloud/voice.html`) 변경 금지 — 외부 플랫폼·틱톡 영상에 박힘
